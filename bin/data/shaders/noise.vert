...
void main(){
    ...
    float maxLen = 900*distortAmount3;
	if( len < maxLen && len > 0  ){
		...
		pos.x += d.x * pct * 200.0f ;
		pos.y += d.y * pct * 200.0f ;
	}
    vec4 col = gl_Color;

	//modify our position with the smooth noise
	pos.x += noiseAmntX * 20.0 *distortAmount1;
    
    vec2 pos2D =vec2(pos.x,pos.y);
    vec2 pos2Dinv =vec2(pos.y,pos.x);
    
    pos.x+=rand2(pos2D);
	pos.y += noiseAmntY * 10.0 *distortAmount1;
...
//        pos.x+=distortAmount4*100.;
        if(mod(floor(pos.x),5)==0){
            pos.y +=rand2(pos2D)*distortAmount4*500;
            pos.y -=rand2(pos2Dinv)*distortAmount4*500;
...
//	col.b += noiseB;
    col.a += noiseB*distortAmount1;
    col.a+=distortAmount2;
